...
KAFDROP_DIR="$CERTS_DIR/kafdrop"
KAFKA_UI_DIR="$CERTS_DIR/kafka-ui"

#KEYCLOAK_DIR="$CERTS_DIR/keycloak"
#NGINX_DIR="$CERTS_DIR/nginx"

mkdir -p "$CA_DIR" "$CTRL1_DIR" "$BROKER1_DIR" "$TRUST_DIR" "$CONFL_SCHEMA_REGISTRY_DIR" "$CONNECT_DIR" "$KAFDROP_DIR" "$KAFKA_UI_DIR"
...
KAFDROP_KEYSTORE_PASSWORD="${KAFDROP_KEYSTORE_PASSWORD:-}"
KAFKA_UI_CONNECT_KEYSTORE_PASSWORD="${KAFKA_UI_CONNECT_KEYSTORE_PASSWORD:-}"
...
if [ ! -f "$CA_DIR/root-kafka-ca.key" ]; then
  openssl genpkey -algorithm RSA \
    -pkeyopt rsa_keygen_bits:4096 \
    -out "$CA_DIR/root-kafka-ca.key"
fi

if [ ! -f "$CA_DIR/root-kafka-ca.crt" ]; then
  openssl req -x509 -new -key "$CA_DIR/root-kafka-ca.key" -sha384 -days 3650 \
    -subj "/C=$COUNTRY/ST=$STATE/L=$LOCALITY/O=$ORG_NAME/OU=Security/CN=$CA_CN" \
    -out "$CA_DIR/root-kafka-ca.crt"
fi

ROOT_CA_CRT="$CA_DIR/root-kafka-ca.crt"
ROOT_CA_KEY="$CA_DIR/root-kafka-ca.key"
...
generate_node_cert "kafdrop" "$KAFDROP_DIR" "$KAFDROP_CONNECT_KEYSTORE_PASSWORD"
generate_node_cert "kafka-ui" "$KAFKA_UI_DIR" "$KAFKA_UI_CONNECT_KEYSTORE_PASSWORD"
