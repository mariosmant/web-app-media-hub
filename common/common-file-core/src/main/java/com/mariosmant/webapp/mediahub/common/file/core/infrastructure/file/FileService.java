...
public class FileService implements FilePort {
     ...
     */
    @Override
    public void writeInputStreamToFile(InputStream inputStream, Path destination) throws IOException {
        Files.createDirectories(destination.getParent());

        int streamBuffer = DEFAULT_STREAM_BUFFER;
        int ioBuffer = DEFAULT_IO_BUFFER;

        try (InputStream in = new BufferedInputStream(inputStream, streamBuffer);
